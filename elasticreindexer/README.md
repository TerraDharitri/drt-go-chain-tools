...
populated by the Numbat nodes from the genesis until the current time).
...
- The `config.toml` file contains by default all the Elasticsearch indices that are populated by an Numbat observing-squad.
...
_**WARN**: Start the observing-squad only after the indices `accounts`, `accountsdcdt` and `tokens` are copied._
...
2. Run multiple instances of `elasticreindexer` each with the next configurations for `indices-with-timestamp`:
    ...
    `a.` `indices-with-timestamp = ["accountsdcdt", "tokens", "dcdts"]`
    ...
    `e.` `indices-with-timestamp = ["accountsdcdthistory"]`
...
and the instance from the point `a.` finished to copy `accountsdcdt` and `tokens` indices._

***

## Audience

This tool should be as generic as possible, and it shouldn't have any custom code related to Numbat instances
of Elasticsearch.
